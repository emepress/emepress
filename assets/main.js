    /*
    const hamburger   = document.getElementById('hamburger');
    const overlay     = document.getElementById('mobileOverlay');
    const mainPanel   = document.getElementById('panel-main');
    const closeBtn    = overlay.querySelector('[data-close]');
    const backButtons = overlay.querySelectorAll('[data-back]');

    function openOverlay() {
      overlay.classList.add('open');
      overlay.classList.remove('level-1');
      overlay.classList.add('level-0');
      overlay.setAttribute('aria-hidden', 'false');
      hamburger.setAttribute('aria-expanded', 'true');
      overlay.querySelectorAll('.panel[data-panel]').forEach(p => p.classList.remove('active'));
    }
    function closeOverlay() {
      overlay.classList.remove('open', 'level-0', 'level-1');
      overlay.setAttribute('aria-hidden', 'true');
      hamburger.setAttribute('aria-expanded', 'false');
    }
    function openSubpanel(id){
      overlay.classList.remove('level-0');
      overlay.classList.add('level-1');
      overlay.querySelectorAll('.panel[data-panel]').forEach(p => {
        p.classList.toggle('active', p.getAttribute('data-panel') === id);
      });
    }
    function backToMain(){
      overlay.classList.remove('level-1');
      overlay.classList.add('level-0');
      overlay.querySelectorAll('.panel[data-panel]').forEach(p => p.classList.remove('active'));
    }

    hamburger.addEventListener('click', openOverlay);
    closeBtn.addEventListener('click', closeOverlay);
    backButtons.forEach(btn => btn.addEventListener('click', backToMain));
    mainPanel.addEventListener('click', (e)=>{
      const trigger = e.target.closest('[data-sub]');
      if(!trigger) return;
      e.preventDefault();
      openSubpanel(trigger.getAttribute('data-sub'));
    });
    window.addEventListener('keydown', (e)=>{
      if(e.key === 'Escape' && overlay.classList.contains('open')) closeOverlay();
    });



    // ===== Overlay blur para mega menú (desktop) =====
const desktopOverlay   = document.getElementById('submenuOverlay');
const navEl            = document.querySelector('nav');
const desktopMenuItems = document.querySelectorAll('nav .menu-item');

function isDesktop(){
  return window.matchMedia('(min-width: 901px)').matches;
}
function openDesktopOverlay(){
  if(!isDesktop()) return;
  desktopOverlay.classList.add('open');
  desktopOverlay.setAttribute('aria-hidden', 'false');
}
function closeDesktopOverlay(){
  desktopOverlay.classList.remove('open');
  desktopOverlay.setAttribute('aria-hidden', 'true');
}

// Abre overlay al entrar en cualquier item que tenga submenu
desktopMenuItems.forEach(li => {
  li.addEventListener('mouseenter', () => {
    if (isDesktop() && li.querySelector('.submenu')) openDesktopOverlay();
  });
});

// Cierra al salir del nav (hover fuera)
navEl.addEventListener('mouseleave', () => {
  if (isDesktop()) closeDesktopOverlay();
});

// Cierra si el usuario pasa el ratón o hace click en el overlay
desktopOverlay.addEventListener('mouseenter', closeDesktopOverlay);
desktopOverlay.addEventListener('click', closeDesktopOverlay);

// (Opcional) Cerrar con Escape
window.addEventListener('keydown', e => {
  if (e.key === 'Escape' && desktopOverlay.classList.contains('open')) {
    closeDesktopOverlay();
  }
});


// Carrusel con arrastre + inercia (momentum)
const track = document.getElementById('macCarousel');

let isDown = false;
let startX = 0;
let startScroll = 0;
let lastX = 0;
let velocity = 0;
let rafId = null;

// helpers
const momentum = () => {
  // fricción: menor = más largo
  velocity *= 0.95;
  track.scrollLeft -= velocity;

  if (Math.abs(velocity) > 0.1) {
    rafId = requestAnimationFrame(momentum);
  } else {
    cancelAnimationFrame(rafId);
    track.classList.remove('dragging'); // vuelve el snap
  }
};

const pointerDown = (e) => {
  isDown = true;
  track.classList.add('dragging');

  // posición inicial del puntero y scroll
  startX = e.clientX ?? e.touches?.[0].clientX;
  lastX  = startX;
  startScroll = track.scrollLeft;
  velocity = 0;

  // evita selección/click fantasma
  e.preventDefault?.();
};

const pointerMove = (e) => {
  if (!isDown) return;

  const clientX = e.clientX ?? e.touches?.[0].clientX;
  const dx = clientX - lastX;

  // mover el scroll en sentido opuesto al arrastre
  track.scrollLeft -= dx;

  // guardar velocidad para la inercia
  velocity = dx;
  lastX = clientX;
};

const pointerUp = () => {
  if (!isDown) return;
  isDown = false;

  // iniciar inercia
  cancelAnimationFrame(rafId);
  rafId = requestAnimationFrame(momentum);
};

// Mouse
track.addEventListener('mousedown', pointerDown);
window.addEventListener('mousemove', pointerMove);
window.addEventListener('mouseup', pointerUp);

// Touch
track.addEventListener('touchstart', pointerDown, { passive: true });
track.addEventListener('touchmove', pointerMove, { passive: false });
track.addEventListener('touchend', pointerUp);
track.addEventListener('touchcancel', pointerUp);

const track = document.getElementById('macCarousel');

let isDown = false;
let startX;
let scrollLeft;

track.addEventListener('touchstart', (e) => {
  isDown = true;
  startX = e.touches[0].pageX;
  scrollLeft = track.scrollLeft;
});

track.addEventListener('touchmove', (e) => {
  if (!isDown) return;
  const x = e.touches[0].pageX;
  const walk = startX - x; // cuánto arrastras
  track.scrollLeft = scrollLeft + walk;
});

track.addEventListener('touchend', () => {
  isDown = false;
});

*/
/*---------------------------------------------------------------------------------------------------------- */
// ===== Menú hamburguesa móvil =====
const hamburger   = document.getElementById('hamburger');
const overlay     = document.getElementById('mobileOverlay');
const mainPanel   = document.getElementById('panel-main');
const closeBtn    = overlay.querySelector('[data-close]');
const backButtons = overlay.querySelectorAll('[data-back]');

function openOverlay() {
  overlay.classList.add('open');
  overlay.classList.remove('level-1');
  overlay.classList.add('level-0');
  overlay.setAttribute('aria-hidden', 'false');
  hamburger.setAttribute('aria-expanded', 'true');
  overlay.querySelectorAll('.panel[data-panel]').forEach(p => p.classList.remove('active'));
}
function closeOverlay() {
  overlay.classList.remove('open', 'level-0', 'level-1');
  overlay.setAttribute('aria-hidden', 'true');
  hamburger.setAttribute('aria-expanded', 'false');
}
function openSubpanel(id){
  overlay.classList.remove('level-0');
  overlay.classList.add('level-1');
  overlay.querySelectorAll('.panel[data-panel]').forEach(p => {
    p.classList.toggle('active', p.getAttribute('data-panel') === id);
  });
}
function backToMain(){
  overlay.classList.remove('level-1');
  overlay.classList.add('level-0');
  overlay.querySelectorAll('.panel[data-panel]').forEach(p => p.classList.remove('active'));
}

hamburger.addEventListener('click', openOverlay);
closeBtn.addEventListener('click', closeOverlay);
backButtons.forEach(btn => btn.addEventListener('click', backToMain));
mainPanel.addEventListener('click', (e)=>{
  const trigger = e.target.closest('[data-sub]');
  if(!trigger) return;
  e.preventDefault();
  openSubpanel(trigger.getAttribute('data-sub'));
});
window.addEventListener('keydown', (e)=>{
  if(e.key === 'Escape' && overlay.classList.contains('open')) closeOverlay();
});

// ===== Overlay blur para mega menú (desktop) =====
const desktopOverlay   = document.getElementById('submenuOverlay');
const navEl            = document.querySelector('nav');
const desktopMenuItems = document.querySelectorAll('nav .menu-item');

function isDesktop(){
  return window.matchMedia('(min-width: 901px)').matches;
}

function openDesktopOverlay(){
  if(!isDesktop()) return;
  desktopOverlay.classList.add('open');
  desktopOverlay.setAttribute('aria-hidden', 'false');
}
function closeDesktopOverlay(){
  desktopOverlay.classList.remove('open');
  desktopOverlay.setAttribute('aria-hidden', 'true');
}

desktopMenuItems.forEach(li => {
  li.addEventListener('mouseenter', () => {
    if (isDesktop() && li.querySelector('.submenu')) openDesktopOverlay();
  });
});

navEl.addEventListener('mouseleave', () => {
  if (isDesktop()) closeDesktopOverlay();
});

desktopOverlay.addEventListener('mouseenter', closeDesktopOverlay);
desktopOverlay.addEventListener('click', closeDesktopOverlay);

window.addEventListener('keydown', e => {
  if (e.key === 'Escape' && desktopOverlay.classList.contains('open')) {
    closeDesktopOverlay();
  }
});

// ===== Carrusel adaptado escritorio / móvil =====
const track = document.getElementById('macCarousel');
let isDown = false, startX = 0, startScroll = 0, lastX = 0, velocity = 0, rafId = null;

function initCarousel() {
  // Limpia listeners previos
  track.replaceWith(track.cloneNode(true));
  const newTrack = document.getElementById('macCarousel');

  if(isDesktop()) {
    // Desktop: arrastre con inercia
    const momentum = () => {
      velocity *= 0.95;
      newTrack.scrollLeft -= velocity;
      if (Math.abs(velocity) > 0.1) {
        rafId = requestAnimationFrame(momentum);
      } else {
        cancelAnimationFrame(rafId);
        newTrack.classList.remove('dragging');
      }
    };

    const pointerDown = (e) => {
      isDown = true;
      newTrack.classList.add('dragging');
      startX = e.clientX ?? e.touches?.[0].clientX;
      lastX = startX;
      startScroll = newTrack.scrollLeft;
      velocity = 0;
      e.preventDefault?.();
    };

    const pointerMove = (e) => {
      if (!isDown) return;
      const clientX = e.clientX ?? e.touches?.[0].clientX;
      const dx = clientX - lastX;
      newTrack.scrollLeft -= dx;
      velocity = dx;
      lastX = clientX;
    };

    const pointerUp = () => {
      if (!isDown) return;
      isDown = false;
      cancelAnimationFrame(rafId);
      rafId = requestAnimationFrame(momentum);
    };

    newTrack.addEventListener('mousedown', pointerDown);
    window.addEventListener('mousemove', pointerMove);
    window.addEventListener('mouseup', pointerUp);

    newTrack.addEventListener('touchstart', pointerDown);
    newTrack.addEventListener('touchmove', pointerMove);
    newTrack.addEventListener('touchend', pointerUp);
    newTrack.addEventListener('touchcancel', pointerUp);

  } else {
    // Móvil: arrastre horizontal solo, sin inercia
    newTrack.addEventListener('touchstart', (e) => {
      isDown = true;
      startX = e.touches[0].pageX;
      startScroll = newTrack.scrollLeft;
    });

    newTrack.addEventListener('touchmove', (e) => {
      if(!isDown) return;
      const x = e.touches[0].pageX;
      const walk = startX - x;
      newTrack.scrollLeft = startScroll + walk;
    });

    newTrack.addEventListener('touchend', () => {
      isDown = false;
    });
  }
}

// Inicializa carrusel
initCarousel();

// Re-inicializa si cambia el tamaño / rotación
window.addEventListener('resize', initCarousel);
